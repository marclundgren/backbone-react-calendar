...
app.CalendarSources = React.createClass({displayName: 'CalendarSources',
  ...
  onChange: function(e) {
    ...
    var filteredEvents = vals.map(function(name) {
      debugger;
      // return self.props.eventscollection.where({calendarName: item})
      return self.props.eventscollection.filter(function(item) {
        // debugger
        console.log('item: ', item.get('calendarName'));
        console.log('name: ', name);
        return name === item.get('calendarName');
      });
    });
